<?php
    ...
    if(isset($_REQUEST['client_id']) && $_REQUEST['client_id'] !== '' && !isset($_REQUEST['submit_btn']) && !isset($_REQUEST['submit_back'])){
        ...
        header("Location: clientsrequests.php?edit=1&id=$id&$queryString");
        exit();
    } else if(isset($_REQUEST['client_id']) && $_REQUEST['client_id'] !== '' && isset($_REQUEST['details']) && $_REQUEST['details'] !== '' && (isset($_REQUEST['submit_btn']) || isset($_REQUEST['submit_back']))){
        $id = filter_input(INPUT_POST, 'id', FILTER_SANITIZE_NUMBER_INT);
        $client_id = filter_input(INPUT_POST, 'client_id', FILTER_SANITIZE_NUMBER_INT);
        
        $flag = '0';
        $action = "تم اضافة طلب للموكل : <br> رقم الموكل : $client_id<br>";
        ...
        if(isset($file_id) && $file_id !== ''){
            ...
            if($resultcheck->num_rows == 0){
                header("Location: clientsrequests.php?edit=1&id=$client_id&error=fid");
                exit();
            }
            if(isset($file_id) && $file_id !== ''){
                $flag = '1';
        ...
        $stmtcheck = $conn->prepare("UPDATE clients_requests SET client_id=?, file_id=?, subject=?, details=?, date=?, timestamp=? WHERE id=?");
        $stmtcheck->bind_param("iissssi", $client_id, $file_id, $subject, $details, $date, $timestamp, $id);
        $stmtcheck->execute();
        ...
        if(isset($submit_back) && $submit_back === 'addmore'){
            header("Location: clientsrequests.php?requestedited=1&cid=$client_id&addmore=1");
            exit();
        } else{
            header("Location: clientsrequests.php?requestedited=1&cid=$client_id");
            exit();
        }
    } else{
        $id = filter_input(INPUT_POST, 'id', FILTER_SANITIZE_NUMBER_INT);
        
        header("Location: clientsrequests.php?edit=1&id='$id'");
        exit();
    }
?>
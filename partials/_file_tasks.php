...
/**
 * Partial view for managing tasks in the file edit form.
 *
 * Uses variables from FileEdit.php:
 * - $data['tasks'], $data['job_names'], $data['file_degrees'], $data['researchers'] (arrays)
 * - $row_permcheck (array)
 * - $fileId (int)
 */
?>
...
<section class="form-section">
    <div class="section-header">
        <h2>المهام (<?php echo count($data['tasks']); ?>)</h2>
    </div>
            ...
            <tbody>
                ...
                <!-- Existing Tasks -->
                 <?php if (empty($data['tasks'])): ?>
                    <tr><td colspan="6">لا توجد مهام مسجلة.</td></tr>
                <?php else: ?>
                    ...
                    <tr class="status-<?php echo $task['task_status']; ?>">
                        ...
                        <td>
                            <?php 
                                // Find job name from the pre-fetched array
                                $task_name = 'N/A';
                        ...
                        <td>
                             <?php 
                                $emp_name = 'N/A';
                                // This could be any user, not just a researcher. A full user list might be better.
                        ...
                        <td class="actions-cell">
                            <?php if($row_permcheck['admjobs_eperm'] == 1): ?>
                                <a href="?id=<?php echo $fileId; ?>&tid=<?php echo $task['id']; ?>" class="action-btn" title="تعديل"><i class='bx bx-edit'></i></a>
                            <?php endif; ?>
                            <?php if($row_permcheck['admjobs_dperm'] == 1): ?>
                                <a href="editfile.php?tid=<?php echo $task['id']; ?>&fid=<?php echo $fileId; ?>" class="action-btn delete" onclick="return confirm('هل أنت متأكد؟')"><i class='bx bx-trash'></i></a>
                            <?php endif; ?>
                        </td>
                    </tr>
                    <?php endforeach; ?>
